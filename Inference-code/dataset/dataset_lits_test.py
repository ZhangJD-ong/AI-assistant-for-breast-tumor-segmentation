...
def Test_all_Datasets(dataset_path,size):
    ...
    for file in data_list:
        ...
        print("\nStart Evaluate: ", file)
        x0,_,_ = normalization(load(os.path.join(dataset_path,file,'P0.nii.gz')))
        x0 = x0.astype(np.float32)

        t0 = 1
        x1,x_min,x_max = normalization(load(os.path.join(dataset_path, file, 'P1.nii.gz')))
        x1 = x1.astype(np.float32)
        t1 = 1

        if os.path.exists(os.path.join(dataset_path,file,'P2.nii.gz')):
            x2 = normalization_fix(load(os.path.join(dataset_path, file, 'P2.nii.gz')),x_min,x_max).astype(np.float32)
            t2 = 1
        else:
            x2 = x1
            t2 = 0

        if os.path.exists(os.path.join(dataset_path,file,'P3.nii.gz')):
            x3 = normalization_fix(load(os.path.join(dataset_path, file, 'P3.nii.gz')),x_min,x_max).astype(np.float32)
            t3 = 1
        else:
            x3 = x1
            t3 = 0
        if os.path.exists(os.path.join(dataset_path,file,'P4.nii.gz')):
            x4 = normalization_fix(load(os.path.join(dataset_path, file, 'P4.nii.gz')),x_min,x_max).astype(np.float32)
            t4 = 1
        else:
            x4 = x1
            t4 = 0

        if os.path.exists(os.path.join(dataset_path,file,'P5.nii.gz')):
            x5 = normalization_fix(load(os.path.join(dataset_path, file, 'P5.nii.gz')),x_min,x_max).astype(np.float32)
            t5 = 1
        else:
            x5 = x1
            t5 = 0

        itkimage = sitk.ReadImage(os.path.join(dataset_path, file, 'P1.nii.gz'))
